...
| `clickAction`                  | `session`                      | What clicking a notification does. `session` opens the matching Claude Code session; `window` only brings the VSCode window to the front — no session, chat, or folder lookup. Useful with `.code-workspace` setups or VSCode forks.                   |
| `clearOnFocus`                 | `true`                         | Remove delivered notifications from Notification Center when you focus the VSCode window they belong to. macOS only.                                                                                                                                   |
...
- **Click opens the repo folder instead of focusing my window.** Happens when the session-open command isn't available (VSCode forks) or the repo lives inside a `.code-workspace`. Set `claudeCodeSupernotifier.clickAction` to `window` so a click only brings the VSCode window to the front.
