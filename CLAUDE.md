...
- **MUST NOT** swallow errors silently in TS code that runs inside the extension host — surface them via `vscode.window.showErrorMessage` or rethrow. Hook code (which Claude reads on stderr) MAY swallow non-fatal logging errors; everything written to disk must already wrap in `try`.
